namespace :parse do
    desc "Parse socialsites"
        task :facebook,[:keyword] => :environment do  |t, args|
            ...
            browser.goto("m.facebook.com")
            browser.text_field(name: "email").set('arma23430@gmail.com')
            browser.text_field(name: "pass").set('arsenalsuper1900')    
            browser.button(name: "login").click
            browser.button(type:"submit").click
            browser.goto("www.facebook.com/search/str/"+args.keyword+"/keywords_blended_posts?filters=eyJycF9hdXRob3IiOiJ7XCJuYW1lXCI6XCJtZXJnZWRfcHVibGljX3Bvc3RzXCIsXCJhcmdzXCI6XCJcIn0ifQ%3D%3D")
            sleep 1
            index=0
            body_index=0
            comments_index=0
...
=begin
                while index<=10
                    browser.element(class:'_o02').click
                    sleep 1
...
#=begin
            while index<=100
                browser.element(class:'_o02').click
                sleep 2
                if browser.element(class:"_5pbx",index:index).exist?
                    body=browser.element(class:"_5pbx",index:index).text
                    index=index+1
                    Post.create(text: body)
                end
            end
#=end
